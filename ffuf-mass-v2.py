    ...
    for line in read:
        ...
        if 'http://' not in line and 'https://' not in line:
            ...
            if args.wordlist !=None:
                if args.output !=None:
                    cmd = "ffuf -u %s -w %s -mc 200 -fs 42 -c -v -ac -of html -o %s/%s.html"%(j,args.wordlist,args.output,out)
                else:
                    cd = os.getcwd()
                    cmd = "ffuf -u %s -w %s -mc 200 -fs 42 -c -v -ac -of html -o %s/%s.html"%(j,args.wordlist,cd,out)
